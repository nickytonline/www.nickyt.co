PR_TITLE="chore (automated): update talks"
BRANCH_NAME="chore_automated_update_talks_$(date +%s)"
...
if [[ -n "$(git status --porcelain)" ]]; then
  ...
  gh pr create --title "$PR_TITLE" --body "This is an automated PR to update talks"
