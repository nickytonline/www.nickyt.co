PR_TITLE="chore (automated): update newsletter archives"
BRANCH_NAME="chore_automated_update_newsletter_archives_$(date +%s)"
...
if [[ -n "$(git status --porcelain)" ]]; then
  ...
  gh pr create --title "$PR_TITLE" --body "This is an automated PR to update newsletter archives"
